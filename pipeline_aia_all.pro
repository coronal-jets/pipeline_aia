...
foreach wave, config.waves, i do begin
    ...
    if ~keyword_set(method) then begin
        pipeline_aia_find_candidates_m0, work_dir, aia_dir_wave_sel[i], wave, obj_dir, config, files_in, presets_file = presets_file
    endif else begin    
        pipeline_aia_find_candidates, work_dir, aia_dir_wave_sel[i], wave, obj_dir, config, files_in, presets_file = presets_file
    endelse    
    t0 = systime(/seconds)
